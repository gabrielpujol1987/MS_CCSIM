...
ms_level = 1;				% Number of MultiScale level (ONLY: 1, 2, 3).


inputPath = 'C:\Users\gfarina\Documents\GitHub\MS_CCSIM\tis\';

resultsFolderPath = ['Experiment Batch Normalized 2017-04-12 - mincut ACTIVATED, ' num2str(ms_level) 'MS, n=' num2str(nSims) '\'];
...
% 		ms_level = 3;			% Number of MS level (1, 2, 3).
		...
		experiment_folder_name = [num2str(xSize(i)) 'x' num2str(ySize(i)) ' T=' num2str(TT(i)) ' OL=' num2str(OOLL(i)) ' MS=' num2str(ms_level)];
		experimentPath = [baseTIPath 'MS-CCSIM ' experiment_folder_name '\'];
		if ~(exist(experimentPath, 'dir') == 7)
			mkdir(experimentPath);
		end
		
		
		timeTablePath = [experimentPath 'TimeTable ' experiment_folder_name '.csv'];
		fileID = fopen(timeTablePath,'w');
		fprintf(fileID,'MS-CCSIM ');
		fprintf(fileID,[experiment_folder_name '\n']);		
