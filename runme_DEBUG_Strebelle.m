...
xSize =    [400];	% X size of Simulation grid (Ex. 1000x1000)
ySize =    [400];	% Y size of Simulation grid (Ex. 1000x1000)
TT    =    [16];	% Template size which should be even
OOLL  =    [4 ];	% Overlap size which should be even
nSims =    5;			 		% Number of realizations
...
nfiles = 1;                             % waaaaaaaaaaaaaaaaaaaa
...
% 		ms_level = 3;			% Number of MS level (1, 2, 3).
		...
		hd(5,5) = 1;
		% Main Function
		[C_2D, ~] = CCSIM_2D_Gabriel_rasterPath(ti, hd, T, OL, CT, fc, prop, rad, cand, mrp, T_vibration, ms_level(i), real_numb);	% location_2D
