...
function [Grid_Sim, LOC] = CCSIM_2D_MS2_Gabriel(TI, hd, T, OL, CT, fc, prop, cand)
...
cntr = 0;
a = numel(1:T-OL:sizeout(1)-T+1);	% Grid_Sim
b = numel(1:T-OL:sizeout(2)-T+1);	% Grid_Sim
LOC = NaN(a*b,2);
...
for i=[1:T-OL:sizeout(1)-T, sizeout(1)-T+1],
  for j=[1:T-OL:sizeout(2)-T, sizeout(2)-T+1],
	  ...
	  if (i > 1) && (j > 1)		% inverted L shape overlap
	  ...
	  elseif i > 1				% vertical overlap |
          shared = Grid_Sim(i:i+OL-1,j:j+T-1);
          CC = CCtop - 2 * xcorr2(TI2, shared) + sum(shared(:).^2);
          CC = CC(OL:end-T+1,T:end-T+1);
	  
	  elseif j > 1				% horizontal overlap --
        shared = Grid_Sim(i:i+T-1,j:j+OL-1);
        CC = CCside - 2 * xcorr2(TI2, shared) + sum(shared(:).^2);
        CC = CC(T:end-T+1,OL:end-T+1);
      
	  else						% initial case (RANDOM)
          CC = rand(size(TI2,1)-T,size(TI2,2)-T);
	  end;
      
      % CC is a matrix of 'similarity'?
	  
	  
	  
	  
	  
	  if hd_indicator==0 && HD_indicator==0	% if there is no Hard Data
          % get the positions of the 'cand' greatest values from CC
		  [~,loc] = sort(CC(:));
          [ibest, jbest] = ind2sub(size(CC),loc(1:cand,1));	
		  
          if fc~=0		% just selects a random position among the 'cand'
              [c, ~] = hist_cat(TI, Grid_Sim, T, OL, fc, ibest, jbest, i, j);
		  else			
              c = ceil(rand * length(ibest));
          end;
          
		  % I think this is the position of the selected Pattern.
		  pos = [ibest(c) jbest(c)];
