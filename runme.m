...
% 
% % ti = read_eas_sq('tis\Bangladesh.sgems');
% ti = read_eas_sq('tis\ti_strebelle.sgems');
% 
% % ti = read_eas_sq('tis\C_WLTICAT.sgems');
% % ti = read_eas_sq('tis\A_WLreferenceCAT.sgems');
% 
% nx_sim = 250;
% ny_sim = 250;
% 
...
% OL = 249;			% Overlap size which should be even
% T = 250;%100;		% Template size which should be even
...
% 
% % path = 'C:\Users\gfarina\Desktop\Realizations\WildCat\';
% % path = 'C:\Users\gfarina\Desktop\Realizations\AWild\';
% path = 'C:\Users\gfarina\Desktop\Realizations\teste flip\';
% if ~(exist(path, 'dir') == 7)
% 	mkdir(path);
% end
...
hd = NaN(nx_sim,ny_sim,nz_sim); % Simulation grid (180x140x30)
...
for i = 1:real_numb % for each realization:
%  b = reshape(C_2D(:,i), nx_sim, ny_sim); % do the reshape business.
 b = reshape(C_3D(:,i), nx_sim,ny_sim,nz_sim);
 csvwrite([path 'MATRIX_3D_' int2str(i) '.csv'], b); % write it down to the CSV (:,:,i)
%  
%  % Normalizing the values of B, to get the GRAY tones!!!
%  maxInMatrix = max(b(:));
%  b(:,:,:) = b(:,:,:)/maxInMatrix;
